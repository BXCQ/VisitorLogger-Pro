                        ...
                        <div id="post-content" class="wrapper-lg">
                            ...
                            <div class="visitor-stats-container">
                                ...
                                <div id="loadingStatus" class="filter-status" style="color: #666; padding: 6px; margin-bottom: 10px; background: #f8f9fa; border-radius: 4px;">
                                    <p>统计数据：过滤条件 - 全部时间 <span class="excluded-note">(已排除管理员登录设备IP)</span></p>
                                </div>
                                    ...
                                    <div class="stats-card-header">
                                        ...
                                        <div class="header-controls">
                                            <div class="chart-style-toggle">
                                                <button class="active" data-style="ring">环形图</button>
                                                <button data-style="rose">玫瑰图</button>
                                                <button data-style="3d">3D图</button>
                                            </div>
...
.list-view {
    ...
    -webkit-overflow-scrolling: touch; /* 添加弹性滚动，支持触摸滑动 */
...
@media (max-width: 768px) {
    ...
    /* 确保移动端图表正确显示 */
    #countryChart {
        height: 550px !important; /* 进一步增加移动端高度 */
        width: 100% !important;
    }
...
@media (max-width: 480px) {
    ...
    .stats-card {
        min-height: 550px; /* 增加小屏幕最小高度 */
    }
    
    /* 确保小屏幕图表正确显示 */
    #countryChart {
        height: 500px !important; /* 增加小屏幕高度 */
    }
...
.dark .date-inputs input[type="date"] {
...
// 客户端排除检测 - 添加进页面加载时检测
echo "// 检查当前会话是否应该被排除\n";
echo "if (document.cookie.indexOf('visitor_stats_excluded=1') >= 0) {\n";
    ...
    document.querySelectorAll('.view-toggle button').forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (view === 'list') {
                ...
                // 如果没有指定或不是list，回到图表视图
                listView.classList.remove('active');
                chartView.classList.add('active');
                
                // 恢复样式切换按钮的激活状态
                this.classList.remove('active-view');
...
// 更新显示筛选结果的函数
function updateFilterStatus(startDate, endDate, totalVisits, countriesCount, excludedCount) {
    ...
    if (loadingStatus) {
        ...
        if (startDate && endDate) {
            ...
            statusText = `统计数据：${formattedStart} 至 ${formattedEnd}，共 ${totalVisits || 0} 次访问，来自 ${countriesCount || 0} 个国家/地区`;
        } else {
            statusText = `统计数据：全部时间`;
        }
        
        // 添加排除提示
        if (excludedCount > 0) {
            statusText += ` <span class="excluded-note">(已排除管理员登录设备IP: ${excludedCount} 次访问, ${excludedIPs.length} 个IP)</span>`;
        } else {
            statusText += ` <span class="excluded-note">(已排除管理员登录设备IP)</span>`;
        }
        
        loadingStatus.innerHTML = `<p>${statusText}</p>`;
...
function filterDataByDate() {
    ...
    const filteredLogsWithoutExcluded = filteredLogs.filter(log => {
        if (excludedIPs.includes(log.ip)) {
            excludedCount++;
            return false;
        }
        return true;
    ...
    for (const country in countryStats) {
        ...
        if (countryIpStats[country]) {
            ...
            for (const ip in countryIpStats[country]) {
                ipArray.push({ ip: ip, count: countryIpStats[country][ip] });
            }
    ...
    updateFilterStatus(startDate, endDate, totalVisits, countriesData.length, excludedCount);
}

// 过滤排除的IP
function filterExcludedIPs(data) {
    if (!data || !data.rawData) return { countries: [] };
    ...
    try {
        ...
        originalStatsData = <?php
            ...
            try {
                ...
                // 获取所有记录用于统计
                $logs = $db->fetchAll($db->select('country, COUNT(*) as count')
                    ->from($prefix . 'visitor_log')
                    ->group('country')
                ...
                echo json_encode([
                    'countries' => [
                        ...
                        ['country' => '中国河南省移动', 'count' => '754', 'ips' => ['117.23.45.67 (235)', '117.24.56.78 (215)']],
                        ['country' => '澳大利亚', 'count' => '287', 'ips' => ['203.23.45.67 (98)', '203.24.56.78 (92)']],
                        ['country' => '中国广东省电信', 'count' => '226', 'ips' => ['119.23.45.67 (88)', '119.24.56.78 (68)']]
                    ],
                    'rawData' => [
                        ['ip' => '112.23.45.67', 'country' => '中国福建省电信', 'time' => '2023-05-01 10:30:45'],
                        ['ip' => '58.23.45.67', 'country' => '中国福建省联通', 'time' => '2023-05-02 11:25:13'],
                        ['ip' => '117.23.45.67', 'country' => '中国河南省移动', 'time' => '2023-05-03 14:45:22'],
                        ['ip' => '203.23.45.67', 'country' => '澳大利亚', 'time' => '2023-05-04 16:12:08'],
                        ['ip' => '119.23.45.67', 'country' => '中国广东省电信', 'time' => '2023-05-05 09:34:56']
                    ]
                ]);
            }
        ?>;
        ...
        setTimeout(function() {
            if (typeof filterDataByDate === 'function') {
                ...
                // 如果filterDataByDate不可用，至少更新排除信息
                console.log('更新排除信息...');
                updateFilterStatus(null, null, 
                    globalStatsData.totalVisits, 
                    globalStatsData.countries.length,
...
function updateChartStyle(style) {
    ...
    countryChart.setOption({
        ...
        series: [{
            ...
            labelLine: {
                length: style === 'rose' ? 15 : 10,
                length2: style === 'rose' ? 15 : 10,
                smooth: true
            },
            emphasis: {
                focus: 'series',
